...
def create_faculty(faculty: FacultyModel):
...
@router.post("/grades/")
def create_grade(grade: GradeModel):
...
def update_faculty(faculty_id: int, faculty: FacultyModel):
    handler.update_faculty(faculty_id, faculty.name)
    return {"message": "Факультет обновлен"}
...
def update_grade(grade_id: int, grade: GradeModel):
